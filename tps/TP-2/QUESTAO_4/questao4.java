...
public class questao4 {
    ...
    public static void insertionSort(Veiculo[] veiculos, int n){
        for(int i = 1; i < n; i++){
            Veiculo atual = veiculos[i]; // guarda o veiculo que vai ser encaixado
            int j = i - 1;
            while(j >= 0 && veiculos[j].getMarca().compareTo(atual.getMarca()) > 0){
                veiculos[j + 1] = veiculos[j]; // empurra pra frente pra abrir espaco
                j--;
            }
        veiculos[j + 1] = atual; // encaixa na posicao certa
        }
    }
    public static void main(String[] args){
        ...
        Veiculo[] selecionados = new Veiculo[veiculos.length]; // array para armazenar os veiculos selecionados
        int qtdSelecionados = 0;
        while(id != -1){ 
            for(int i = 0; i < veiculos.length; i++){
                if(veiculos[i].id == id){
                    selecionados[qtdSelecionados] = veiculos[i]; // guarda em vez de printar
                    qtdSelecionados++;
                    break;
                }
            }
        ...
        sc.close();
        insertionSort(selecionados, qtdSelecionados); // ordena os veiculos selecionados por marca  
        for(int i = 0; i < qtdSelecionados; i++){
        System.out.println(selecionados[i].format()); // imprime os veiculos formatados
        }   
    }
}